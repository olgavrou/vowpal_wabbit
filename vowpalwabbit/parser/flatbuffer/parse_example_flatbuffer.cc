...
{
  // new example/multi example object to process
...
void parser::parse_multi_example(vw* all, example* ae, const MultiExample* eg)
{
  ...
  { parse_flat_label(all->sd, ae, eg->shared_type(), eg->shared()); }
  else if (eg->label_type() != VW::parsers::flatbuffer::Label_NONE)
  {
    if (eg->example_w_label() == _multi_ex_index)
    {
      cost_count++;
    ...
    shared_data* sd, example* ae, const VW::parsers::flatbuffer::Label& label_type, const void* label)
{
  switch (label_type)
  {
    ...
    {
      const SimpleLabel* simple_label = static_cast<const SimpleLabel*>(label);
