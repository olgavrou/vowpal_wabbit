...
template <typename It>
int generate_pdf_given_chosen_action(It pdf_first, It pdf_last, float chosen_action, uint32_t min_value,
    uint32_t max_value, uint32_t num_actions, uint32_t bandwidth, float epsilon)
{
  ...
  /**
   *
   * pdf_lim will hold the pdf limits/intervals
   * pdf_lim will have minimum size 3 and maximum 4 for any combination of num_actions (i.e. K) and bandwidth
   * if action falls into the first range (min_value to min_value + unit_range) then pdf_lim will look like: {0, action + bandwidth, K - 1}
   * if action falls into the last range (max_value - unit_range to max_value) then pdf_lim will look like: {0, action - bandwidth, K - 1}
   * if action falls anywhere in between, then pdf_lim will look like: {0, action, action + bandwith, K - 1}
   *
   **/
  std::vector<uint32_t> pdf_lim;
  pdf_lim.push_back(0);

  if (action == num_actions - 1) { pdf_lim.push_back(action - bandwidth); }

  if (action - bandwidth >= 0 && action != 0) { pdf_lim.push_back(action); }

  if (action + bandwidth <= num_actions - 1) { pdf_lim.push_back(action + bandwidth); }

  if (pdf_lim.back() != num_actions - 1) pdf_lim.push_back(num_actions - 1);

  /**
   * 
   * Integral over the pdf from min_value to max_value must equal to 1
   * Using epsilon greedy, the chosen action interval has (1 - e) probability of
   * being selected and the rest e is divided among the other intervals.
   * 
   * Here we calculate the value of the pdf (p') of the chosen interval to be (1 / (bandwidth * unit_range))
   * and the value of the pdf (p') of the non-chosen regions to be zero.
   * 
   * Thus, when we calculate the final pdf_value for each interval it will be:
   * chosen action pdf_value: (1 - e)*p' + e / (max_value - min_value)
   * non-chosen interval pdf_values: (1 - e)*p' + e / (max_value - min_value) = e / (max_value - min_value)
   * 
   * Integrating over the pdf values using the above boundaries of pdf_lim, which are calculated using the chosen action and
   * provided bandwith, the integral over the pdf from min_value to max_value will be equal to 1
   * 
   **/
  ...
  {
    if (pdf_it == pdf_last) break;
    float p = 0;
    const float left = min_value + pdf_lim[i] * unit_range;
    const float right = min_value + pdf_lim[i + 1] * unit_range;
    pdf_it->left = left;
    pdf_it->right = right;
    if (pdf_lim[i] == action || (pdf_lim[i + 1] == action && i == m - 2) /* action is K - 1  */)
    {
      p += 1.f / (bandwidth * unit_range);
    }
    pdf_it->pdf_value = p * (1 - epsilon) + epsilon / (max_value - min_value);
    ++pdf_it;
  }
