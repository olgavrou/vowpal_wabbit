...
void convert(const std::string& from, const std::string& to)
{
  ...
  std::ofstream prefix_file;
  prefix_file.open(".prefix_file.txt");
  ...
  while (std::getline(data_file, line))
  {
    ...
    prefix_file << size << " " << static_cast<int>(object_type::example) << std::endl;
    ...
    outfile.write(reinterpret_cast<char*>(buf), size);
    prefix_file << size << " " << static_cast<int>(object_type::example_collection) << std::endl;
  }
